...
function Calendar(props) {
  ...
  const [animationHeight, setAnimationHeight] = useState(new Animated.Value(40));
    ...
    () => {
      if (props.zoomOut) {
        Animated.parallel([
          ...
          Animated.timing(animationHeight, { toValue: 0.01, duration: 100 })
        ]).start();
      } else {
        Animated.parallel([
          Animated.timing(animationOpacity, {
          ...
          Animated.timing(animationHeight, { toValue: 55, duration: 100 })
        ]).start();
      }
    },
    [props.zoomOut]
      ...
      <View style={{ padding: 20, backgroundColor: "white" }}>
        <View style={{ flexDirection: "row", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
          <View style={{ flexDirection: "row", alignItems: "flex-end" }}>
            ...
            <Animated.Text style={{ opacity: animationOpacity, marginLeft: 5, color: "gray" }}>
              {datePicker.getDay() == 0 ? <Text>Chủ nhật</Text> : <Text>Thứ {datePicker.getDay() + 1}</Text>}
            </Animated.Text>
          </View>
